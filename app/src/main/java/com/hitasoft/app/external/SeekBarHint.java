...
public class SeekBarHint extends AppCompatSeekBar implements SeekBar.OnSeekBarChangeListener {
    ...
    private void showPopup() {
        ...
        if (mPopupStyle == POPUP_FOLLOW) {
            mPopup.showAtLocation(sView, Gravity.LEFT, (int) (sView.getX() + (int) getXPosition(this)), (int) (sView.getY() + mYLocationOffset + sView.getHeight()));
        }
        if (mPopupStyle == POPUP_FIXED) {
            mPopup.showAtLocation(sView, Gravity.CENTER | Gravity.BOTTOM, 0, (int) (sView.getY() + mYLocationOffset + sView.getHeight()));
        }
    }
